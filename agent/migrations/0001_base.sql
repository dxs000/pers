-- Базовая миграция: схема, с которой проект жил до Шага 34.
--
-- **SQL ниже не тронут ни на утверждение** — файл переехал `git mv`, а не
-- был переписан «раз уж всё равно правим». Это условие принятия: база,
-- поднятая прежним `db.py --init` из `schema.sql`, обязана считаться
-- применившей ровно эту миграцию, иначе `--stamp` записал бы в журнал
-- неправду и следующая миграция легла бы на схему, которой не ожидала.
--
-- Шапка при этом переписана, и это безопасно ровно сегодня: сверка идёт
-- контрольной суммой ФАЙЛА, но суммы этой пока нет ни в одном журнале —
-- журнала не существовало до этого шага. С первым `--stamp` окно
-- закрывается: дальше правка любого символа в этом файле, включая
-- комментарий, будет расхождением, и `--check` назовёт его.
--
-- Что снято из прежней шапки и почему: строка «миграций нет, старт чистый».
-- Она была верна, пока в базе не лежало ничего содержательного, и перестала
-- быть верной ровно тогда, когда персонажу разрешили жить в фоне. Цена
-- сноса теперь не в пересборке эталонов, а в биографии, которую
-- восстановить неоткуда.
--
-- Проверка схемы — по-прежнему не «таблицы создались», а промпты,
