...
def build_system_prompt(
    ...
    # Побуждения (Шаг 56) — рядом с незакрытым, но не тем же блоком: нить —
    # дело, о котором можно заговорить, побуждение — то, откуда он смотрит.
    # Страх, выложенный первой репликой, делает из персонажа пациента, и
    # формулировка блока держит это словами. Пусто — блока нет, как у нитей.
    import drives as drives_mod
    drives_block = drives_mod.render_for_self(turn.drives)
    if drives_block:
...
# Шаг 63: рассказанное помечается. Человек с памятью знает, что эту историю
# он уже рассказывал, — и не рассказывает её заново как новость. Без пометки
# модель видит строку впервые на каждом ходу, и повтор ей ничем не заметен.
TOLD_MARK = "(уже рассказывал) "


def _memory_mark(m: dict) -> str:
    told = TOLD_MARK if m.get("told_at") else ""
    return told + MEMORY_MARKS.get(m.get("source") or "", "")


# =============================================================================
# Рассказал ли (Шаг 63)
# =============================================================================
# Показ в промпте — не вспоминание (`0019_surfacing.sql`). Вспоминание — когда
# ответ строку ЗАДЕЛ. Решает это не модель, а сверка слов: ещё один вызов на
# каждую реплику ради бита «упомянул / не упомянул» — дорого, а ошибка здесь
# дешёвая (лишняя или пропущенная прибавка веса 0.1).
#
# Основы — первые пять букв слов от пяти букв: грубая русская стемминга,
# которой хватает, чтобы «кутаисский двор» узнался в «во дворе в Кутаиси».
# Служебные длинные слова отсекаются списком, иначе «потому», «который» и
# «сейчас» совпадали бы у всего со всем.
TOLD_STEM = 5
TOLD_MIN_SHARED = 2
TOLD_MIN_SHARE = 0.3
_TOLD_STOP = {
    "котор", "потом", "тольк", "когда", "сейча", "здесь", "очень", "всегд",
    "почем", "потому", "может", "будет", "было", "этого", "этому", "своег",
    "моего", "между", "после", "перед", "тогда", "никог", "никто", "ничег",
    "снило", "помню", "совсе", "какой", "какая", "какие", "такой", "такая",
}
...
def _stems(text: str) -> set[str]:
    ...
    return {w[:TOLD_STEM] for w in words
            if len(w) >= TOLD_STEM and w[:TOLD_STEM] not in _TOLD_STOP}


def told_memories(memories, answer: str) -> list[dict]:
    """Какие из показанных воспоминаний ответ задел. Пусто — обычный исход."""
    said = _stems(answer)
    if not said:
        return []
    out = []
    for m in memories or ():
        own = _stems(m.get("text") or "")
        if not own:
            continue
        shared = len(own & said)
        if shared >= TOLD_MIN_SHARED and shared / len(own) >= TOLD_MIN_SHARE:
            out.append(m)
    return out
...
IMPULSE_REASONS = {
    ...
    # Шаг 55. Строка переехала сюда из `news.attach_reason`, который дописывал
    # её в словарь при импорте модуля. Повод, чья формулировка появляется
    # только если кто-то импортировал `news`, — это повод, который в другом
    # процессе (инспектор, сбруя) рендерился бы заглушкой «Есть о чём сказать».
    "news": "Ты глянул в новости, и одно задело. Ленту не пересказывай.",
    # Эссе заводит повод, когда закончено или брошено (Шаг 53), но строки
    # для него не было, и персонаж получал безликое «Есть о чём сказать»
    # вместо того, чтобы знать, что толкнуло. Предмет — название.
    "essay": "Ты дописал или бросил то, что писал сам.",
    # Шаг 57: поводы, которые заводят его собственные дела.
...
def _render_deeds(deeds: dict | None) -> str | None:
    ...
    **Порции одной книги складываются.** Два захода по десять страниц — это
    ...
    # Дела по своей воле (Шаг 57). Вспомнившееся уже лежит в каноне своей
    # строкой, поэтому здесь только сам факт; задуманное НЕ выдаётся за
    # случившееся — это мысль, и строка так и говорит.
    for p in deeds.get("pursuits") or []:
        about = f" ({p['about']})" if p.get("about") else ""
        outcome = (p.get("outcome") or "").strip()
        if p["action"] == "explore":
            lines.append(f"- полез разбираться{about}"
                         + (f": {clip_text(outcome, DEED_NOTE_LIMIT)}" if outcome else ""))
        elif p["action"] == "recall":
            lines.append(f"- вспоминал{about}")
        elif p["action"] == "daydream":
            lines.append(f"- задумался{about} - это были мысли, а не события")
...
def _build_day_prompt(turn: Turn, canon: list[dict], born: datetime | None,
    ...
    # Шаг 56: чего он хочет и боится. День не обязан это исполнять — но
    # человек, который боится остаться без работы, и вечер проводит иначе.
    import drives as drives_mod
    drives_block = drives_mod.render_about(turn.drives, "Чего он хочет и чего боится:")
    if drives_block:
        parts.append(drives_block + "\n")
...
def _build_dream_prompt(turn: Turn, canon: list[dict], born: datetime | None,
    ...
    # Побуждения (Шаг 56) — туда же, в остаток. Сны перерабатывают именно
    # это: чего хочется и чего страшно, — и отдельный блок с заголовком
    # «главное» превратил бы сон в иллюстрацию к нему.
    if turn.drives:
        import drives as drives_mod
...
# Выбор книги (`choose_book` и его промпт) жил здесь до Шага 56 копией того,
# что демон на самом деле звал из `shelf_choice`. Копия удалена: один проход —
# одно место, и сбруя проверяет то, что работает.
