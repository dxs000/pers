...
def _build_summarizer_prompt(turn: Turn, buffer: dict) -> str | None:
    ...
    items = (buffer or {}).get("exchanges") or []
    if not items:
        return None
    ...
    lines = []
    for item in items[-SUMMARY_EXCHANGES_LIMIT:]:
        lines.append(f"Собеседник: {clip_text(item.get('user', ''))}")
        lines.append(f"{name}: {clip_text(item.get('assistant', ''))}")
    transcript = "\n".join(lines)
