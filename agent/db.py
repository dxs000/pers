...
def pending_ids(conn) -> list[str]:
    """Какие миграции из репозитория в базе не применены. Для демона (Шаг 64.1).

    Код и схема едут вместе: код Шага N зовёт функции, которые заводит
    миграция Шага N. Демон, запущенный без неё, не падает — он пишет в журнал
