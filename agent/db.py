...
# `followups` держит отложенный T2 (Шаг 27): очередь переваривания, из
# которой `cycle.digest_one` берёт работу. В списке её не было до Шага 33, и
# пропуск был не косметический — база без неё проходила `--check` целиком, а
# падала на первом же ходу, на `enqueue_digest`, то есть уже ПОСЛЕ ответа
# человеку. Проверка, зеленеющая ровно там, где сломается работа, хуже
# отсутствующей: она отводит подозрение от схемы.
EXPECTED_TABLES = {
    "objects", "aliases", "assertions", "episodes",
    "sessions", "messages", "agent", "inbox", "followups",
}
